...
if [ ! -f "/var/www/html/wp-config.php" ]; then
  ...
  wp cli --allow-root --debug update
  ...
  /usr/local/bin/wp --allow-root --debug config set FS_METHOD 'direct'
  # Instala efetivamente o WordPress (cria admin e define a URL).
  /usr/local/bin/wp --allow-root --debug core install --url="https://${WORDPRESS_HOST}" --title="${WORDPRESS_TITLE}" --admin_user="${WORDPRESS_ADM_NAME}" --admin_password="${WORDPRESS_ADM_PASS}" --admin_email="${WORDPRESS_ADM_EMAIL}"
  /usr/local/bin/wp --debug --allow-root user create "${WORDPRESS_USERNAME}" "${WORDPRESS_USER_EMAIL}" --role="editor" --user_pass="${WORDPRESS_USER_PASS}"
